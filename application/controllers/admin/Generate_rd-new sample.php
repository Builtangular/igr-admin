...
class Generate_rd extends CI_Controller {    
	...
	public function rd_1()
	{
		...
		if($this->session->userdata('logged_in')){
			...
			$filename = "".htmlspecialchars($new_file_name4)."-RD1.docx";
    ...
    public function rd_2()
	{
		...
		if($this->session->userdata('logged_in')){
			...
			$new_file_name4=str_replace("--","-", $new_file_name3);
			$objWriter = PHPWord_IOFactory::createWriter($PHPWord, 'Word2007');
			$filename = "".htmlspecialchars($new_file_name4)."-RD2.docx";
			$objWriter->save($filename);
			header('Content-Description: File Transfer');
			header('Content-Type: application/octet-stream');
			header('Content-Disposition: attachment; filename='.$filename);
	...
	{		
		...
		if($this->session->userdata('logged_in')){
			...
			$new_file_name4=str_replace("--","-", $new_file_name3);
			$objWriter = PHPWord_IOFactory::createWriter($PHPWord, 'Word2007');
			$filename = "".htmlspecialchars($new_file_name4)."-TOC.docx";
			$objWriter->save($filename);
			header('Content-Description: File Transfer');
			header('Content-Type: application/octet-stream');
			header('Content-Disposition: attachment; filename='.$filename);
	...
	/* merge pre-existing templates with created files */
	public function merge_sample_pages(){
		if($this->session->userdata('logged_in')){
			$session_data = $this->session->userdata('logged_in');
			$data['Login_user_name'] = $session_data['Login_user_name'];	
			$data['Role_id'] = $session_data['Role_id'];
			
			$templateFile  = "sample/sample_template.docx";
			$generatedFile = "sample/content-file.docx";
			$targetFile    = "FinalMergeResult.docx";
	...
	public function new_sample_pages(){
		if($this->session->userdata('logged_in')){
			$session_data = $this->session->userdata('logged_in');
			$data['Login_user_name']=$session_data['Login_user_name'];	
			$data['Role_id']=$session_data['Role_id'];
			...
			if($scope_name == 'Global'){
				$desc_scope_name = strtolower($scope_name);
				$scope_type = 'Region';
				$scope_typeforpara2 = 'Regional';
				$geographic = 'geographic regions';
			} else {
				$desc_scope_name = $scope_name;
				$scope_type = 'Country';
				$scope_typeforpara2 = 'Country';
			...
			$New_sample_description_para1_2=str_replace("Value_unit",ucwords($Value_unit),$New_sample_description_para1_1);
			...
			$New_sample_description_para2_7 = str_replace("Value_unit",ucwords($Value_unit),$New_sample_description_para2_6);
			$New_sample_description_para2_8 = str_replace("Segment_name",strtolower($rd_segments_name),$New_sample_description_para2_7);
			$New_sample_description_para2_9 = str_replace("Global_regional_line_only",$desc_scope_name." as well as ".strtolower($scope_typeforpara2),$New_sample_description_para2_8);
			...
			$New_sample_description_para2_11 = "The market analysis covers major ".$geographic.", such as ".$rd_region_name.".";
			$New_sample_description_para2_12 = $New_sample_description_para2_9." ".$New_sample_description_para2_11;
			// var_dump($New_sample_description_para2_12); die;
			if($scope_name == 'Global'){
				$New_sample_description_para2_13 = $New_sample_description_para2_12." The market size and forecasts are also given for these regions. Countries with major market revenues are covered for each of the regions.";
			} else {
            ...
            if($scope_name != 'Global'){
				$New_sample_description_para3_2 = str_replace("world","region",$New_sample_description_para3_2);
			} 
			...
			if($Volume_unit){
				$Get_research_approaches_para1_4 = str_replace("Value_unit",ucwords($Value_unit).' and in terms of volume in '.strtolower($Volume_unit),$Get_research_approaches_para1_3);
			}else{
				$Get_research_approaches_para1_4 = str_replace("Value_unit",ucwords($Value_unit),$Get_research_approaches_para1_3);
			}
			...
			if($scope_name != 'Global'){
				$Get_research_approaches_para3_3 = str_replace("world","region",$Get_research_approaches_para3_3);
			} 
			...
			// var_dump($primary_research_para); die;
			...
			$PHPWord = new PhpWord();
			// $section = $PHPWord->addSection();
			// $section = $PHPWord->addSection(array('marginLeft'=>720, 'marginRight'=>720,'marginTop'=>1140,'marginBottom'=>950, 'orientation' => 'landscape'));
			...
			/*----------------------Header -------------------------------*/	
			/* $PHPWord->addParagraphStyle('headerTitle', array('align'=>'left', 'spacing' => 72, 'spaceBefore' => 160, 'spaceAfter' => 80));
			$PHPWord->addFontStyle('headerTitleFont', array('align'=>'left','name'=>'Franklin Gothic Medium', 'color'=>'#ffffff', 'size' => 9, 'italic' =>true));
			$header = $section->addHeader(); */
			// $header->addImage('images/sample-header.png', array('width' => 780, 'height' => 40, 'alignment' => \PhpOffice\PhpWord\SimpleType\Jc::START));	
			...
			$header->addTextRun(array('align' => 'right'))->addImage('images/sample-header.png', $options);	
			$table = $header->addTable();
			$table->addRow();					
			$table->addCell(1800, array())->addText(htmlspecialchars(),'headerTitleFont', 'headerTitle');
			
			$table->addCell(150, array())->addText('');

			$table->addCell(1900, array())->addText('');
			
			$table->addCell(150, array())->addText('');

			$table->addCell(13000, array())->addText(htmlspecialchars(ucwords(' '.$report_title).": ".htmlspecialchars($scope_name)." Industry Analysis, Trends, Market Size and Forecasts up to ".$forecast_to),'headerTitleFont', 'headerTitle'); */

			/* header with code */
			/* $table = $header->addTable();
			$table->addRow();	
			$table->addCell(1800, array('borderBottomSize' => 15, 'borderBottomColor' => '#E59C24', 'CellMarginLeft'  => 1.5, 'CellMarginRight' => 0.5, 'CellMarginTop'   => 0.5,  'CellMarginBottom'=> 0.5, array('space' => array('before' => 160, 'after' => 80)), 'bgcolor' => '#17365D'))->addText(htmlspecialchars(),'headerTitleFont', 'headerTitle');
			
			$table->addCell(150, array('marginLeft'  => 0.5, 'marginRight' => 0.5, 'marginTop'   => 0.5,  'marginBottom'=> 0.5, array('space' => array('before' => 160, 'after' => 80))))->addText('');

			$table->addCell(1700, array('marginLeft'  => 0.5, 'marginRight' => 0.5, 'marginTop'   => 0.5,  'marginBottom'=> 0.5, array('space' => array('before' => 160, 'after' => 80))))->addImage('images/new_logo.png', array('width'=>80, 'height'=>27, 'positioning' => 'relative'));
			
			$table->addCell(150, array('marginLeft'  => 0.5, 'marginRight' => 0.5, 'marginTop'   => 0.5,  'marginBottom'=> 0.5, array('space' => array('before' => 160, 'after' => 80))))->addText('');

			$table->addCell(13000, array('borderBottomSize' => 15, 'borderBottomColor' => '#E59C24', 'borderMarginTop'  => 1.5, 'CellMarginLeft'  => 0.5, 'CellMarginRight' => 0.5, 'CellMarginTop'   => 0.5,  'CellMarginBottom'=> 0.5, array('space' => array('before' => 160, 'after' => 80)), 'bgcolor' => '#17365D'))->addText(htmlspecialchars(ucwords(' '.$report_title).": ".htmlspecialchars($scope_name)." Industry Analysis, Trends, Market Size and Forecasts up to ".$forecast_to),'headerTitleFont', 'headerTitle');  
			...
			$PHPWord->addParagraphStyle('footerTitle', array('spacing' => 72, 'spaceBefore' => 120, 'spaceAfter' => 240, 'color'=>'#fff', 'align'=> $textAlign ::CENTER));
			$PHPWord->addFontStyle('footerTitleFont', array('name'=>'Franklin Gothic Medium', 'color'=>'#78777C', 'size' => 10.5));
			$PHPWord->addFontStyle('footerPageFont', array('name'=>'Franklin Gothic Medium', 'color'=>'#78777C', 'size' => 10.5));
			...
			$footer->addTextRun(array('align' => 'right'))->addImage('images/sample-footer.png', $options);
			$table = $footer->addTable();
			$table->addRow();
			$copyright = "© Infinium Global Research";
			$table->addCell(14700, array('marginLeft'  => 0.5, 'marginRight' => 0.5, 'marginTop' => 0.5,  'marginBottom' => 0.5))->addText(html_entity_decode($copyright), 'footerTitleFont', 'headerTitle');
			$table->addCell(1100, array())->addPreserveText('Page {PAGE}', 'footerPageFont', 'footerTitle');
			$table->addCell(1300, array('marginLeft'  => 0.5, 'marginRight' => 0.5, 'marginTop'   => 0.5,  'marginBottom' => 0.5))->addText('', 'footerTitleFont', 'headerTitle');  */
			/*----------------------Footer -------------------------------*/
			...
			/* Cover page */
			$section->addText(htmlspecialchars(strtoupper('								')), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#00435E', 'size' => 30, 'bold' => true));
			$section->addText(htmlspecialchars(strtoupper('								')), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#00435E', 'size' => 30, 'bold' => true));
			$section->addText(htmlspecialchars(strtoupper('								')), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#00435E', 'size' => 30, 'bold' => true));
			$section->addText(htmlspecialchars(strtoupper('								')), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#00435E', 'size' => 30, 'bold' => true));
			/* $section->addText(htmlspecialchars(strtoupper('								')), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#00435E', 'size' => 30, 'bold' => true));
			$section->addText(htmlspecialchars(strtoupper('								')), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#00435E', 'size' => 30, 'bold' => true));
			$section->addText(htmlspecialchars(strtoupper('								')), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#00435E', 'size' => 30, 'bold' => true)); */
			$section->addText(htmlspecialchars(strtoupper('								'.$report_title)), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#00435E', 'size' => $font_size, 'bold' => true));
			$section->addText(strtoupper('								'.$scope_name).' INDUSTRY ANALYSIS, TRENDS, MARKET SIZE AND FORECASTS UP TO '.strtoupper($forecast_to), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#7F7F7F', 'size' => 11));

			$section->addImage('images/new/cover-img.jpg',['positioning' => 'relative','marginTop' => 0,'marginLeft' => 1,'width' => 845,'height' => 605,'wrappingStyle' => 'behind']);
			$section->addPageBreak();
			$section->addImage('images/new/about-us.jpg',['positioning' => 'relative','marginTop' => 0,'marginLeft' => 1,'width' => 850,'height' => 605,'wrappingStyle' => 'tight']);
			$section->addImage('images/new/services.jpg',['positioning' => 'relative','marginTop' => 0,'marginLeft' => 1,'width' => 850,'height' => 605,'wrappingStyle' => 'tight']);
			/* Cover page */
			
			/* Paragraph Styles */
			$PHPWord->addFontStyle('Style1', array('align'=>'left','name'=>'Roboto Condensed','size'=>22,'color'=>'#E59C24', 'bold' => true, 'pageBreakBefore' => true));
			...
			$PHPWord->addFontStyle('ChapStyle', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>15,'color'=>'#E59C24', 'bold' => false));
			$PHPWord->addFontStyle('subpoint', array('bold'=>false, 'name'=>'Franklin Gothic Medium','size' => 12, 'color'=>'#78777C'));
			$PHPWord->addFontStyle('childpoint', array('bold'=>false, 'name'=>'Franklin Gothic Medium','size' => 11, 'color'=>'#626262'));
			...
			if($Volume_unit){
				$section->addText('	2.1	'.htmlspecialchars(ucwords($Report_title_scope, " \t\r\n\f\v'")).' Highlights, (USD '.htmlspecialchars(ucwords($Value_unit)).', '.htmlspecialchars($Volume_unit).')', 'subpoint', 'P-Style');
			}else{
				$section->addText('	2.1	'.htmlspecialchars(ucwords($Report_title_scope, " \t\r\n\f\v'")).' Highlights, (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'subpoint', 'P-Style');
			}
			$section->addText('	2.2	'.htmlspecialchars(ucwords($Report_title_scope, " \t\r\n\f\v'")).' Projection', 'subpoint', 'P-Style');
			if($scope_name == 'Global'){
				$section->addText('	2.3	'.htmlspecialchars(ucwords($Report_title_scope, " \t\r\n\f\v'")).' CAGR, (USD '.htmlspecialchars(ucwords($Value_unit)).') Growth, By Regions', 'subpoint', 'P-Style');
			...
			foreach($main_segments as $segments)
			{
				$new_segment=$Report_title_scope.' by '.htmlspecialchars(ucwords($segments->name, " \t\r\n\f\v'"));	
				if($Volume_unit){
					$section->addText($chap.'.	'.htmlspecialchars($new_segment).' (USD '.htmlspecialchars(ucwords($Value_unit)).', '.htmlspecialchars($Volume_unit).')', 'ChapStyle', 'PStyle');
				}else{
					$section->addText($chap.'.	'.htmlspecialchars($new_segment).' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'ChapStyle', 'PStyle');
				}
				...
				foreach($sub_segments as $subsegments)
				{
					...
					if($child_segments)
					{
						foreach($child_segments as $childsegments)
						{
							...
							$subchildpt = 1;
							$sub_child_segments = $this->RdData_model->get_rd_segments($report_id, $childsegments->id);
			...
			if($Volume_unit){
				$section->addText($regchapt.'.	'.htmlspecialchars($Report_title_scope).', by '.$scope_type.' (USD '.htmlspecialchars(ucwords($Value_unit)).', '.htmlspecialchars($Volume_unit).")", 'ChapStyle', 'PStyle');
			} else {
				$section->addText($regchapt.'.	'.htmlspecialchars($Report_title_scope).', by '.$scope_type.' (USD '.htmlspecialchars(ucwords($Value_unit)).")", 'ChapStyle', 'PStyle');
			}
			...
			$section->addTextBreak(1);
			$section->addText('TABLE 1   '.htmlspecialchars(strtoupper($Report_title_scope))." HIGHLIGHTS", array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
			$parent = 0;
			$main_segments= $this->RdData_model->get_rd_segments($report_id, $parent);
			$num = 2;
			/* Main segments */
			foreach($main_segments as $segments)
			{
				$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($Report_title_scope))." BY ".htmlspecialchars(strtoupper($segments->name)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
				$num++;

				if($Volume_unit)
				{
					$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($Report_title_scope))." BY ".htmlspecialchars(strtoupper($segments->name)).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
					$num++;
				}
				/* --- sub segments --- */
				$sub_segments= $this->RdData_model->get_rd_segments($report_id, $segments->id);	
				foreach($sub_segments as $subsegments)
				{
					$child_segments = $this->RdData_model->get_rd_segments($report_id, $subsegments->id);
					if($child_segments){
						$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($segments->name).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
						$num++;

						if($Volume_unit)
						{
							$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($segments->name).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
							$num++;
						}
						/* --- child segment--- */
						foreach($child_segments as $childsegments)
						{
							$sub_child_segments = $this->RdData_model->get_rd_segments($report_id, $childsegments->id);
							if($sub_child_segments){
								$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$subsegments->name)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
								$num++;
								if($Volume_unit)
								{
									$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." FOR ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$subsegments->name)).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
									$num++;
								}
								/* --- sub child segment--- */							
								if($sub_child_segments){
									foreach($sub_child_segments as $subchildsegments)
									{
										$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title.' BY '.$subchildsegments->name))." BY ".htmlspecialchars(strtoupper($scope_type)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
										$num++;
										if($Volume_unit)
										{
											$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title.' BY '.$subchildsegments->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
											$num++;
										}
									}
								}
								...
								$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$scope_type)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
								$num++;
								if($Volume_unit)
								{
									$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." FOR ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$scope_type)).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
									$num++;
								}
							}							
						}
						...
						$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title))." BY ".htmlspecialchars(strtoupper($subsegments->name.' BY '.$scope_type)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
						$num++;

						if($Volume_unit)
						{
							$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title))." BY ".htmlspecialchars(strtoupper($subsegments->name.' BY '.$scope_type)).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
							$num++;
						}
					}
				}
			...
			/* /. Main Segments */
			/* Region */
			$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($Report_title_scope))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
			$num++;

			if($Volume_unit)
			{
				$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($Report_title_scope))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
				$num++;
			}
			...
			if($ScRegion != NULL) {
    			...
    			for($i = 0; $i < $j ; $i++)
    			{
    				...
    				{
    					$mainseg = $segments->name;
    					$region_fix = ltrim(rtrim($ScRegion[$i])).' '.htmlspecialchars(ucwords($report_title, " \t\r\n\f\v'"));
    					$region_report_title = ltrim(rtrim($ScRegion[$i])).' '.htmlspecialchars(ucwords($report_title, " \t\r\n\f\v'")).' by '.ltrim(rtrim(ucwords($mainseg)));
    					$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($region_report_title))." ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
    					$num++;
    
    					if($Volume_unit)
    					{
    						$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($region_report_title))." ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
    						$num++;
    					}
    				}
				...
				if($scope_name == 'Global'){
    				if($ScRegion[$i] == "RoW")
    				{
    					$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($region_fix)).', BY SUB-REGION'." ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
    					$num++;
    					if($Volume_unit)
    					{
    						$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($region_fix)).', BY SUB-REGION'." ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
    						$num++;
    					}
    					/* Sub regions */
    					...
    					if($Regcntry1 != NULL){
        					...
        					for($r = 0; $r < $ctry1; $r++)
        					{
        						...
        						{
        							$mainseg = $segments->name;
        							$country_report_title = ltrim(rtrim($Regcntry1[$r])).' '.htmlspecialchars(ucwords($report_title, " \t\r\n\f\v'")).' by '.ltrim(rtrim(ucwords($mainseg)));
        							
        							$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($country_report_title))." ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
        							$num++;
        						}
        					}			
        					unset($Regcntry1);
    					...
    					$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($region_fix)).', BY COUNTRY'." ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
    					$num++;
    					if($Volume_unit)
    					{
    						$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($region_fix)).', BY COUNTRY'." ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
    						$num++;
    					}
    					/* Sub regions */
    					$get_scope_country1 = $this->RdData_model->get_scope_regions($ScRegionId[$i]); 
			...
			$section->addText(htmlspecialchars($figure1), array('align'=>'center','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
			$figure2="FIGURE 2   RESEARCH APPROACHES - BOTTOM UP \n\n";
			$section->addText(htmlspecialchars($figure2), array('align'=>'center','name'=>'Franklin Gothic Medium','color' => '#78777C','size'=>12));
			$figure3="FIGURE 3   RESEARCH APPROACHES - TOP-DOWN  \n\n";
			$section->addText(htmlspecialchars($figure3), array('align'=>'center','name'=>'Franklin Gothic Medium','color' => '#78777C','size'=>12));
			$figure4="FIGURE 4   IGR - RESEARCH METHOD AND DATA TRIANGULATION  \n\n";
			$section->addText(htmlspecialchars($figure4), array('align'=>'center','name'=>'Franklin Gothic Medium','color' => '#78777C','size'=>12));
			if($Volume_unit){
			...
			$section->addText(htmlspecialchars($figure5), array('align'=>'center','name'=>'Franklin Gothic Medium','color' => '#78777C','size'=>12));
			...
			$section->addText(htmlspecialchars($figure6), array('align'=>'center','name'=>'Franklin Gothic Medium','color' => '#78777C','size'=>12));
			$figure7="FIGURE 7   DRIVERS OF ".htmlspecialchars(strtoupper($Report_title_scope)).", IMPACT ANALYSIS \n\n";
			$section->addText(htmlspecialchars($figure7), array('align'=>'center','name'=>'Franklin Gothic Medium','color' => '#78777C','size'=>12));
			/* $figure8="FIGURE 8   RESTRAINTS OF ".htmlspecialchars(strtoupper($Report_title_scope)).", IMPACT ANALYSIS \n\n";
			$section->addText(htmlspecialchars($figure8), array('align'=>'center','name'=>'Franklin Gothic Medium','color' => '#78777C','size'=>12)); */
			$figure9="FIGURE 8   PORTER"."'"."S FIVE FORCES ANALYSIS \n\n";
			$section->addText(htmlspecialchars($figure9), array('align'=>'center','name'=>'Franklin Gothic Medium','color'=>'#78777C', 'size'=>12));
			$figure10="FIGURE 9   IGR- GROWTH MATRIX ANALYSIS";
			$section->addText(htmlspecialchars($figure10), array('align'=>'center','name'=>'Franklin Gothic Medium','color' => '#78777C','size'=>12));
			$number = 10;
			/* Segments */
			foreach($main_segments as $segments)
			{
				$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($Report_title_scope))." BY ".htmlspecialchars(strtoupper($segments->name)).", ".$Base_year." - ".$forecast_to." (REVENUE % SHARE)", array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
				$number++;					
				if($Volume_unit) {
					$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($Report_title_scope))." BY ".htmlspecialchars(strtoupper($segments->name)).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
					$number++;
				} else {
					$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($Report_title_scope))." BY ".htmlspecialchars(strtoupper($segments->name)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
					$number++;
				}
				/* --- sub segment---- */
				$sub_segments= $this->RdData_model->get_rd_segments($report_id, $segments->id);	
				foreach($sub_segments as $subsegments)
				{
					$child_segments = $this->RdData_model->get_rd_segments($report_id, $subsegments->id);
					if($child_segments){
						$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($segments->name).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
						$number++;
						if($Volume_unit)
						{
							$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($segments->name).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
							$number++;
						}	
						/* --- child segment---- */	
						foreach($child_segments as $childsegments)
						{
							$sub_child_segments = $this->RdData_model->get_rd_segments($report_id, $childsegments->id);	
							if($sub_child_segments){
							$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$subsegments->name)).", ".$Base_year." - ".$forecast_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
							$number++;
							if($Volume_unit)
							{
								$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$subsegments->name)).", ".$Base_year." - ".$forecast_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
								$number++;
							}							
								/* --- sub child segment---- */
								foreach($sub_child_segments as $subchildsegments)
								{
									$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($subchildsegments->name.' BY '.$scope_type)).", ".$Base_year." - ".$forecast_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
									$number++;
									if($Volume_unit)
									{
										$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." FOR ".htmlspecialchars(strtoupper($$subchildsegments->name.' BY '.$scope_type)).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
										$number++;
									}
								} /* ./ --- sub child segment---- */
							} else{
								$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$scope_type)).", ".$Base_year." - ".$forecast_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
								$number++;
								if($Volume_unit)
								{
									$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$scope_type)).", ".$Base_year." - ".$forecast_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
									$number++;
								}
							}
						}	/* ./ --- child segment---- */				
					}else{
						$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
						$number++;
						if($Volume_unit)
						{
							$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
							$number++;
						}
					}		
				}
			...
			/* /. Segments */
			/* Regions */
			$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($Report_title_scope))." BY ".strtoupper($scope_type).", ".$Base_year." - ".$forecast_to." (REVENUE % SHARE)", array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));			
			$number++;
			if($Volume_unit){
				$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($Report_title_scope))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
				$number++;
			}else{
				$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($Report_title_scope))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
				$number++;
			}
			$Scope_Region;
			$get_scope_regions= $this->RdData_model->get_scope_regions($scope_id);            
			...
			if($ScReg1 != NULL){
    			...
    			for($i = 0; $i < $j ; $i++)
    			{
    				...
    				{
    					$mainseg = $segments->name;
    					$region_fix = ltrim(rtrim($ScReg1[$i])).' '.htmlspecialchars(ucwords($report_title, " \t\r\n\f\v'"));
    					$region_report_title = ltrim(rtrim($ScReg1[$i])).' '.htmlspecialchars(ucwords($report_title, " \t\r\n\f\v'")).' by '.ltrim(rtrim(ucwords($mainseg)));
    					$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($region_report_title))." ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
    					$number++;
    					if($Volume_unit)
    					{
    						$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($region_report_title))." ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
    						$number++;
    					}
    				}
					/* Scope Global Only */
					if($scope_name == 'Global'){
						if($ScReg1[$i] == "RoW")
						{
							$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($region_fix)).', BY SUB-REGION '.$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
							$number++;
							if($Volume_unit)
							{
								$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($region_fix)).', BY SUB-REGION '.$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
								$number++;
							}
							/* Sub regions */
							...
							if($Regcntry2 != NULL){
								...
								for($r = 0; $r < $ctry; $r++)
								{
									...
									{
										$mainseg = $segments->name;
										$country_report_title = ltrim(rtrim($Regcntry2[$r])).' '.htmlspecialchars(ucwords($report_title, " \t\r\n\f\v'")).' by '.ltrim(rtrim(ucwords($mainseg)));
										
										$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($country_report_title)).' '.$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
										$number++;
									}
								}			
								unset($Regcntry2);
							...
							$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($region_fix)).', BY COUNTRY '.$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
							$number++;
							if($Volume_unit)
							{
								$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($region_fix)).', BY COUNTRY '.$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
								$number++;
							}
							/* Sub regions */
							$get_scope_country2 = $this->RdData_model->get_scope_regions($ScRegId1[$i]); 
			...
			$PHPWord->addFontStyle('chaptHeading', array('align'=>'Left','bold'=>false,'name'=>'Franklin Gothic Medium','color'=>'#E59C24','size'=>18));
			/* ------------------- Chapter Heading Style ----------------------------- */
			...
			$PHPWord->addFontStyle('subPoint', array('align'=>'Left','color'=>'#78777C','bold'=>false,'name'=>'Franklin Gothic Medium','size'=>15));
			/* ------------------- Subpoint Heading Style ----------------------------- */
			...
			$PHPWord->addFontStyle('childPoint', array('align'=>'Left','name'=>'Franklin Gothic Medium','size'=>13,'color'=>'#78777C'));
			/* ------------------- Childpoint Heading Style ----------------------------- */
			...
			$PHPWord->addFontStyle('childSubpoint', array('align'=>'Left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
			/* ------------------- childSubpoint Heading Style ----------------------------- */
			
			/* ------------------- Description Style ----------------------------- */
			$PHPWord->addParagraphStyle('paragraphStyle', array('spacing' => 120, 'lineHeight' => 1.5, 'align'=>'both', 'spaceBefore' => 120, 'spaceAfter' => 240));
			$PHPWord->addFontStyle('r2Style', array('bold'=>false, 'italic'=>false, 'align' => 'both', 'size'=>12.5, 'name'=>'Franklin Gothic Medium'));
			/* ------------------- Description Style ----------------------------- */
			
			/* ------------------- Figure Name Style ----------------------------- */
			$PHPWord->addParagraphStyle('Figure _ Title', array('align'=>'left', 'spacing' => 48, 'spaceBefore' =>0, 'spaceAfter'=>120));
			$PHPWord->addFontStyle('figureNameStyle', array('align'=>'Left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
			/* ------------------- Figure Name Style ----------------------------- */
			
			/* ------------------- Table Name Style ----------------------------- */
			$PHPWord->addParagraphStyle('Table_Title', array('align'=>'left', 'spacing' => 48, 'spaceBefore' =>0, 'spaceAfter'=>240));
			$PHPWord->addFontStyle('tableNameStyle', array('align'=>'Left','name'=>'Franklin Gothic Medium','size'=>12,'color'=>'#78777C'));
			/* ------------------- Table Name Style ----------------------------- */
			...
			foreach($main_segments2 as $segments2)
			{
				...
				$section->addText('Segmentation based on '.htmlspecialchars(ucwords($segments2->name)), array('align'=>'left', 'bold'=>true, 'name'=>'Franklin Gothic Medium', 'color'=>'#000', 'size' => 12.5));
				/* sub segments */
				$sub_segments2= $this->RdData_model->get_rd_segments($report_id, $segments2->id);	
				foreach($sub_segments2 as $subsegments2)
				{
					$section->addListItem(htmlspecialchars($subsegments2->name), 0, array('align'=>'left', 'name'=>'Franklin Gothic Medium', 'color'=>'#000', 'size' => 12.5), $listStyleBullet, 'P-Style');
					/* child segments */
					...
					if($child_segments2)
					{
						...
						{
							$section->addListItem(htmlspecialchars($childsegments2->name), 1, array('align'=>'left', 'name'=>'Franklin Gothic Medium', 'color'=>'#000', 'size' => 12.5), $listStyleBulletEmpty, 'P-Style');
							...
							if($sub_child_segments2)
							{
								...
								{
									$section->addListItem(htmlspecialchars($subchildsegments2->name), 2, array('align'=>'left', 'name'=>'Franklin Gothic Medium', 'color'=>'#000', 'size' => 12.5), $listStyleBulletEmpty, 'P-Style');
								}
							} /* ./ sub child segments */
						}
					}
			...
			$PHPWord->addFontStyle('Source Note', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
			/* -------------------------- Source style ------------------------------ */
			/* -------------------------- Description style ------------------------------ */
			...
			$PHPWord->addFontStyle('trStyle1', array('align'=>'left', 'bold'=>false, 'italic'=>false, 'size'=>11,'name'=>'Franklin Gothic Medium'));
			$PHPWord->addFontStyle('trStyle', array('align'=>'both', 'bold'=>false, 'italic'=>false, 'size'=>11,'name'=>'Franklin Gothic Medium', 'spaceBefore'=>3,'spaceAfter'=>0));
			/* -------------------------- /. Description style ------------------------------ */
			$section->addListItem('Definition, Abbreviations, and Assumptions', 2, 'childPoint', $listStyle, 'Head 2');
    
			// Define cell style arrays
			$styleCell = array('valign'=>'center', 'color' => '#E59C24', 'borderBottomSize'=>2, 'borderTopSize'=>2, 'borderBottomColor'=>'#78777C', 'borderTopColor'=>'#78777C', 'bgColor' => '#112F51');
			$styleRightCellColor = array('bgColor'=>'#112F51');
			...
			$fontStyle = array('bold'=>false, 'align'=>'both','size'=>11,'name'=>'Franklin Gothic Medium','color'=>'#E59C24');
			...
			for($i = 1; $i <= 9; $i++) {
				...
				{
					$table->addRow();
					$table->addCell(3000, $styleRightCellColor)->addText(htmlspecialchars(ucwords($report_title, " \t\r\n\f\v'"))." (Definition)", 'trStyle1', 'pStyle');
					$table->addCell(6500, $styleCellColor)->addText(htmlspecialchars($report_definition), 'trStyle', 'pStyle');
				}
				else if($i==2)
				{
					$table->addRow();
					$table->addCell(3000, $styleRightCellColor)->addText("USD", 'trStyle1', 'Table1_Left');
				...
				{					
					if($scope_name == 'Global'){
						$table->addRow();
						$table->addCell(3000, $styleRightCellColor)->addText("The U.S.", 'trStyle1', 'Table1_Left');
						$table->addCell(6500, $styleCellColor)->addText("United States", 'trStyle', 'pStyle');
					} else if($scope_name == 'MENA'){
						$table->addRow();
						$table->addCell(3000, $styleRightCellColor)->addText("UAE", 'trStyle1', 'Table1_Left');
				...
				{					
					if($scope_name == 'Global'){
						$table->addRow();
						$table->addCell(3000, $styleRightCellColor)->addText("APAC", 'trStyle1', 'Table1_Left');
						$table->addCell(6500)->addText("Asia-Pacific", 'trStyle', 'pStyle');
					} else if($scope_name == 'MENA'){
						$table->addRow();
						$table->addCell(3000, $styleRightCellColor)->addText("MENA", 'trStyle1', 'Table1_Left');
						$table->addCell(6500)->addText("Middle East and North Africa", 'trStyle', 'pStyle');
					} else if($scope_name == 'GCC'){
						$table->addRow();
						$table->addCell(3000, $styleRightCellColor)->addText("GCC", 'trStyle1', 'Table1_Left');
				...
				{					
					if($scope_name == 'Global'){
						$table->addRow();
						$table->addCell(3000, $styleRightCellColor)->addText("RoW", 'trStyle1', 'Table1_Left');
				...
				{
					$table->addRow();
					$table->addCell(3000, $styleRightCellColor)->addText("CAGR", 'trStyle1', 'Table1_Left');
					$table->addCell(6500)->addText("Compound Annual Growth Rate", 'trStyle', 'pStyle');
				}
				else if($i==7)
				{
					$table->addRow();
					$table->addCell(3000, $styleRightCellColor)->addText("Historic Data, Base Year, and Forecast Period", 'trStyle1', 'Table1_Left');
					$table->addCell(6500, $styleCellColor)->addText("The historic year is considered ".$analysis_from.", the base year ".$Base_year." and the forecast period is considered from ".$forecast_period, 'trStyle', 'pStyle');
				}
				else if($i==8)
				{					
					$table->addRow();
					$table->addCell(3000, $styleRightCellColor)->addText("Assumptions", 'trStyle1', 'Table1_Left');
					$table_cell = $table->addCell(6500);
					$table_cell->addText("The market size of the ".htmlspecialchars(strtolower($report_name))." is arrived at considering the regional average prices and pricing trends.",'trStyle1', 'Table Bullet');//0 is the list level
					$table_cell->addText("The market sizing is based on the historic data for ".$From_historic." to ".$analysis_from.", the report however presents ".$Base_year." as a base year, and then the forecast provided is for ".$forecast_from." to ".$forecast_to.'.', 'trStyle1', 'Table Bullet');
				...
				{
					$table->addRow();
					$table->addCell(3000, $styleRightCellColor)->addText("Geographic Coverage by Countries", 'trStyle1', 'Table1_Left');
				...
				{
					$table->addRow();
					$table->addCell(3000, $styleRightCellColor)->addText("Cell $i", 'trStyle1', 'pStyle');
			...
			$section->addImage('images/research_methodology_outline.png', array('width'=>520, 'height'=>380, 'align'=>'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			...
			if($scope_name == 'Global'){
				$primary_research_para_1 = "We have conducted primary interviews with 10-12 key players from the major countries. The key countries considered for the particular market include - The U.S., Germany, France, China, India, Japan, and among others.";
			} else {
				$primary_research_para_1 = "We have conducted primary interviews with 10-12 key players from the major countries. The key countries considered for the particular market include - ".$rd_region_name.".";
			}
			...
			$section->addTextBreak(1);
			$section->addText(htmlspecialchars('Primary Sources Considered During This Particular Study:'), array('align'=>'left', 'bold'=>true, 'name'=>'Franklin Gothic Medium', 'color'=>'#000', 'size' => 12.5));	
			...
			$section->addTextBreak(1);
			$section->addText(htmlspecialchars('Data Points Received Through Primary Research During the Course of Study:'), array('align'=>'left', 'bold'=>true, 'name'=>'Franklin Gothic Medium', 'color'=>'#000', 'size' => 12.5));	
			...
			$section->addText(htmlspecialchars('Breakdown of the Profiles of Primaries as Follows:'), array('align'=>'left', 'bold'=>false, 'name'=>'Franklin Gothic Medium', 'color'=>'#000', 'size' => 12.5));
			if($scope_name == 'Global'){
				$section->addImage('images/new/primaries_breakdown.png', array('width'=>770, 'height'=>220, 'align'=>'center'));
			} else {
				$section->addImage('images/new/primaries_breakdown_country.png', array('width'=>770, 'height'=>220, 'align'=>'center'));
			}			
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addText(htmlspecialchars('Primary interviews not only help in data validation but also provide critical insights into the market, current business scenario, and future expectations and enhance the quality of our reports. In addition to analyzing the current and historical trends, our analysts predict where the market is headed, over the next five years.'), 'r2Style', 'paragraphStyle');
			$section->addTextBreak(1);
			$section->addText(htmlspecialchars('Primary Sources Considered During This Particular Study:'), array('align'=>'left', 'bold'=>true, 'name'=>'Franklin Gothic Medium', 'color'=>'#000', 'size' => 12.5));
			$section->addImage('images/new/primary_sources.png', array('width'=>420, 'height'=>260, 'align'=>'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			...
			$section->addTextBreak(1);
			$section->addText(htmlspecialchars('The Secondary Sources of the Data Typically Include'), array('align'=>'left', 'bold'=>true, 'name'=>'Franklin Gothic Medium', 'color'=>'#000', 'size' => 12.5));
			...
			if($scope_name == 'Global'){							
				$section->addImage('images/new/research_approaches_bottom_up.png', array('width'=>780, 'height'=>320, 'align'=>'center'));
			}else{
				$section->addImage('images/new/research_approaches_bottom_up_region.png', array('width'=>780, 'height'=>320, 'align'=>'center'));
			}
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			...
			if($scope_name == 'Global'){
				if($forecast_period == '2023-2030'){
					$section->addImage('images/new/research_approaches_top_down_2023_2030.png', array('width'=>780, 'height'=>360, 'align'=>'center'));
				} else {
					$section->addImage('images/research_approaches_top_down.png', array('width'=>520, 'height'=>320, 'align'=>'center'));
				} 
			}else{
				$section->addImage('images/new/research_approaches_top_down_2023_2030_region.png', array('width'=>780, 'height'=>320, 'align'=>'center'));
			}
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			...
			$section->addImage('images/new/research_method_and_data_triangulation.png', array('width'=>760, 'height'=>280, 'align'=>'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			...
			if($stack != NULL){
    			...
    			for($i = 1; $i <= count($stack); $i++) {
    				$bg_color = $n % 2 === 0 ? "White" : "#F2F2F2";
    				$styleCellColor = array('bgColor'=>$bg_color);
    				if($i==1)
    				{
    					$table->addRow();
    					$table->addCell(3500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($Report_title_scope, " \t\r\n\f\v'")), 'trStyle1', 'Table1_Left');
    					$table->addCell(3500, $styleCellColor)->addText("Value: ".htmlspecialchars($start_year_revenue.' '.ucwords($Value_unit)), 'trStyle1', 'pStyle');
    					$table->addCell(3500, $styleCellColor)->addText("Value: ".htmlspecialchars($end_year_revenue.' '.ucwords($Value_unit)), 'trStyle1', 'pStyle');
    					$n++;
    					continue;					
    				} 
    				else if($i==2) 
    				{
    					$table->addRow();
    					$table->addCell(3500, $styleRightCellColor)->addText("CAGR (".$forecast_period.")",'trStyle1', 'Table1_Left');
    					if($Volume_unit){
							...
							$styleMergeCell =  array('gridSpan' => 2,'valign' => 'center', 'bgColor'=> $bg_color);
							$table->addcell(7000, $styleMergeCell)->addtext("Value: ".$value_cagr, 'trStyle1', 'pStyle');
    					}
    				...
    				{						
    					...
    					{	
    						...
    						foreach($main_segments3 as $segments3)
    						{
    							...
    							$table->addRow();
    							$table->addCell(3500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($report_title, " \t\r\n\f\v'"))." by ".htmlspecialchars($mainseg3)." Value (USD ".$Value_unit.")",'trStyle1', 'Table1_Left');
    				...
    				else if( $i == count($stack ) ) // Last Element
    				{
    					...
    					$table->addRow();
    					$table_cell = $table->addCell(3500, $styleRightCellColor);
			...
			{
				if($Volume_unit){
					$section->addImage('images/new/market_projection_2021_2030_volume.png', array('width'=>760, 'height'=>320, 'align'=>'center'));
				} else {
					$section->addImage('images/new/market_projection_2021_2030_value.png', array('width'=>760, 'height'=>320, 'align'=>'center'));
				}
				$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
				$section->addTextBreak(1);
			}
			...
			if($scope_name == 'Global'){
				...
				{
					$section->addImage('images/new/market_growth_by_region_2023_2030.png', array('width'=>680, 'height'=>360, 'align'=>'center'));
				}else{
			...
			$section->addImage('images/new/drivers_impact_analysis.png', array('width'=>760, 'height'=>280, 'align'=>'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			/* Restraints */
			...
			$section->addImage('images/new/porters_five_forces.png', array('width'=>760, 'height'=>350, 'align'=>'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			/* style */
			...
			if($mainseg44[0]){
				...
				$section->addListItem(htmlspecialchars(' '.$igr_growth_matrix_segment_title), 2, 'childPoint', $listStyle, 'Head 2');
				$section->addImage('images/new/IGR_growth_matrix_sagment.png', array('width'=>520, 'height'=>320, 'align'=>'center'));
				$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
				$section->addTextBreak(1);
			}
			if($mainseg44[1]) {
			...
			$section->addImage('images/new/value_chain_analysis.png', array('width' => 760, 'height' => 260, 'align' => 'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addPageBreak();	
			/* ./ Sample Pages Chapter 3 */
			...
			foreach($main_segments5 as $segments5)
			{
				...
				if($segment_overview){
					...
					$section->addText(htmlspecialchars($Report_title_scope.' is segmented based on '.$segments5->name.' as '.$subsegmentslist.' XX segment leads the segment and is anticipated to dominate in the forecast period.'), 'r2Style', 'paragraphStyle');
				}
				$section->addTextBreak(1);
			/* /. Segment Overview */
			...
			/* Pie chart figure */
				...
				if($forecast_period == '2021-2027'){
					...
					$section->addImage('images/new/xyz-market-by-segment-revenue-share-2023-2030-Pie.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
				}else{
			...
			/* Bar chart figure */
				...
				if($forecast_period=='2021-2027') {
					...
					// var_dump('images/new/xyz-market-by-segment-revenue-share-2021-2030-bar.png'); die;
					$section->addImage('images/new/xyz-market-by-segment-revenue-share-2021-2030-bar.png', array('width'=>760, 'height'=>280, 'align'=>'center'));
				}else {
			...
			/* Table Writeup -  Main Segment */
				...
				foreach($sub_segments5 as $subsegments5)
				{
					...
					$styleCellColor = array('bgColor'=>$bg_color);
					$table->addRow();
					$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($subsegments5->name)), 'trStyle', 'pStyle');
					...
					$styleLastCell = array('bgColor'=>$bg_color, 'borderBottomColor'=>'#78777C', 'borderBottomSize'=>2);
					$table->addRow();
					$table->addCell(2500, $styleRightCellColor)->addText('Total', 'trStyle', 'pStyle');
				...
				if($Volume_unit)
				{
					...
					foreach($sub_segments5_1 as $subsegments5_1)
					{
						...
						$styleCellColor = array('bgColor'=>$bg_color);
						$table->addRow();
						$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($subsegments5_1->name)), 'trStyle', 'pStyle');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
				...
				foreach($sub_segments5_2 as $subsegments5_2)
				{
					...
					$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subsegments5_2->name))." BY ".strtoupper($segments5->name).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars($Value_unit).')', 'figureNameStyle', 'Figure _ Title');
					$section->addImage('images/new/sample-figure.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
					$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
					$section->addTextBreak(1);
					if($Volume_unit)
					{
						$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subsegments5_2->name))." BY ".strtoupper($segments5->name).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars($Volume_unit).')', 'figureNameStyle', 'Figure _ Title');
						$section->addImage('images/new/sample-figure.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
						$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
						$section->addTextBreak(1);
					}/* /. adding figure */	
					...
					foreach($child_segments3 as $childsegments3)
					{
						...
						$styleCellColor = array('bgColor'=>$bg_color);
						$table->addRow();
						$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($childsegments3->name)), 'trStyle', 'pStyle');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
				...
				/* ./ Table Writeup - Sub Segment */
					...
					/* adding figure */					
					$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subsegments5_2->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars($Value_unit).')', 'figureNameStyle', 'Figure _ Title');
					$section->addImage('images/new/sample-figure.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
					$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
					$section->addTextBreak(1);
					if($Volume_unit)
					{
						$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subsegments5_2->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars($Volume_unit).')', 'figureNameStyle', 'Figure _ Title');
						$section->addImage('images/new/sample-figure.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
						$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
						$section->addTextBreak(1);
					}/* /. adding figure */	
					/* Table Writeup - Sub Segment */
					/* value based table */
					$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subsegments5_2->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars($Value_unit).')', 'tableNameStyle', 'Table_Title');
					// Add table
					$table = $section->addTable('myOwnTableStyle');
					// Add row
					$table->addRow(70);
					...
					foreach($get_scope_regions1 as $scope_region1)
					{
						...
						$styleCellColor = array('bgColor'=>$bg_color);
						$table->addRow();
						$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($scope_region1->name)), 'trStyle', 'pStyle');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
					...
					if($Volume_unit)
					{
						$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subsegments5_2->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars($Volume_unit).')', 'tableNameStyle', 'Table_Title');
						// Add table
						$table = $section->addTable('myOwnTableStyle');
						// Add row
						$table->addRow(70);
						...
						/* Child Segment - Table & Figure Writing started*/								
							...
							if($sub_child_segments3){	
								...
								foreach($sub_child_segments3 as $subchildsegments31)
								{
									...
									$styleCellColor = array('bgColor'=>$bg_color);
									$table->addRow();
									$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($subchildsegments31->name)), 'trStyle', 'pStyle');
									$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
									$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
									$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
									$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
								...
								foreach($sub_child_segments3 as $subchildsegments3)
								{
									...
									$section->addTextBreak(1);		
									/* sample-figure */
									$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subchild_segment3))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars($Value_unit).')', 'figureNameStyle', 'Figure _ Title');
									$section->addImage('images/new/sample-figure.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
									$section->addTextBreak(1);
									/* value basedtTable */
									$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subchild_segment3))." BY ".htmlspecialchars(strtoupper($scope_type)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars($Value_unit).')', 'tableNameStyle', 'Table_Title');
									// Define table style arrays
									$table = $section->addTable('myOwnTableStyle');
									// Add row
									$table->addRow(70);
									...
									// Add more rows / cells
									$n=0;
									$get_scope_regions31 = $this->RdData_model->get_scope_regions($scope_id); 
									foreach($get_scope_regions31 as $scope_regions31)
									{
										$bg_color = $n % 2 === 0 ? "White" : "#F2F2F2";
										$styleCellColor = array('bgColor'=>$bg_color);
										$table->addRow();
										$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($scope_regions31->name)), 'trStyle', 'pStyle');
										$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
										$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
										$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
										$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
									...
									/* volume based table */
									if($Volume_unit){								
										$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subchild_segment3))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars($Volume_unit).')', 'tableNameStyle', 'Table_Title');
										// Define table style arrays
										$table = $section->addTable('myOwnTableStyle');
										// Add row
										$table->addRow(70);
								...
								/* ./ Sub Child Segments */
								$data['sub_child_segments'] = $this->RdData_model->get_rd_segments($report_id, $childsegments3->id);	
								...
								$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($child_segment3))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars($Value_unit).')', 'figureNameStyle', 'Figure _ Title');
								$section->addImage('images/new/sample-figure.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
								$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
								$section->addTextBreak(1);
								if($Volume_unit)
								{
									$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($child_segment3))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars($Volume_unit).')', 'figureNameStyle', 'Figure _ Title');
									$section->addImage('images/new/sample-figure.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
									$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
									$section->addTextBreak(1);
								}

								$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($child_segment3))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars($Value_unit).')', 'tableNameStyle', 'Table_Title');
								// Add table
								$table = $section->addTable('myOwnTableStyle');
								// Add row
								$table->addRow(70);
								...
								foreach($get_scope_regions2 as $scope_regions2)
								{
									...
									$styleCellColor = array('bgColor'=>$bg_color);
									$table->addRow();
									$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($scope_regions2->name)), 'trStyle', 'pStyle');
									$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
									$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
									$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
									$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
								...
								/* volume based table */
								if($Volume_unit){								
									$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($child_segment3))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars($Volume_unit).')', 'tableNameStyle', 'Table_Title');
									// Define table style arrays
									$table = $section->addTable('myOwnTableStyle');
									// Add row
									$table->addRow(70);
									...
									foreach($get_scope_regions3 as $scope_regions3)
									{
										...
										$styleCellColor = array('bgColor'=>$bg_color);
										$table->addRow();
										$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($scope_regions3->name)), 'trStyle', 'pStyle');
										$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
										$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
										$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
										$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
			...
			if($forecast_period == '2021-2027')	{
				...
				if($scope_name == 'Global'){
					$section->addImage('images/new/xyz-market-by-region-2022-2030-Pie.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
				} else {
					$section->addImage('images/new/xyz-market-by-country-2022-2030-Pie.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
				}				
			} else {
			...
			if($forecast_period == '2021-2027')	{
				...
				if($scope_name == 'Global'){
					$section->addImage('images/new/xyz-market-by-region-2021-2030-Bar.png', array('width'=>760, 'height'=>280, 'align'=>'center'));
				} else {
					$section->addImage('images/new/xyz-market-by-country-2021-2030-Bar.png', array('width'=>760, 'height'=>280, 'align'=>'center'));
				}
			} else {
				$section->addText('add image of xyz-market-by-region-Pie', array('size'=>12, 'color'=>'black'));
			}
			...
			foreach($get_scope_regions4 as $scope_region4)
			{
				...
				$styleCellColor = array('bgColor'=>$bg_color);
				$table->addRow();
				$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($scope_region4->name)), 'trStyle', 'pStyle');
				$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
				$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
				$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
				$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
			...
			/* volume based table */
				if($Volume_unit){
					...
					foreach($get_scope_regions5 as $scope_region5)
					{
						...
						$styleCellColor = array('bgColor'=>$bg_color);
						$table->addRow();
						$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($scope_region5->name)), 'trStyle', 'pStyle');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
					...
					foreach($main_segments6 as $segments6)
					{
						...
						if($Volume_unit){
							...
							if($forecast_period =='2021-2027') {
								...
								$section->addImage('images/new/xyz-market-by-segment-revenue-share-2023-2030-Pie.png', array('width'=>760, 'height'=>280, 'align'=>'center'));
							} else {
						...
						foreach($sub_segments8 as $subsegments8)
						{
							...
							$styleCellColor = array('bgColor'=>$bg_color);
							$table->addRow();
							$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($subsegments8->name)), 'trStyle', 'pStyle');
							$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
							$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
							$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
							$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
				...
				if($scope_name == 'Global'){
					if($scoperegion4[$i] == "RoW")
					{
						...
						foreach($get_scope_country4 as $scope_country4)
						{
							...
							$styleCellColor = array('bgColor'=>$bg_color);
							$table->addRow();
							$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($scope_country4->name)), 'trStyle', 'pStyle');
							$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
							$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
							$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
							$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
						...
						if($scopecountryname5 != NULL){
							...
							for($r = 0; $r < $x ; $r++)
							{
								...
								foreach($main_segments7 as $segments7)
								{
									...
									if($Volume_unit)
									{
										...
										if($forecast_period =='2021-2027')
										{
											...
											$section->addText('Add Regional Bar Graph', array('size'=>15));
										}
											$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
											$section->addText('*Note: The above image is only for sample representation. The actual image differs from the above sample image.', 'noteFontFig', 'noteStyle');
											$section->addTextBreak(1);    									
										...
										if($forecast_period =='2021-2027')
										{
											...
											$section->addText('Add Segment Revenue Bar Graph', array('size'=>15));
										}
											$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
											$section->addText('*Note: The above image is only for sample representation. The actual image differs from the above sample image.', 'noteFontFig', 'noteStyle');
											$section->addTextBreak(1);									
								...
								foreach($sub_segments9 as $subsegments9)
								{
									...
									$styleCellColor = array('bgColor'=>$bg_color);
									$table->addRow();
									$table->addCell(2500, $styleRightCellColor)->addText(htmlspecialchars(ucwords($subsegments9->name)), 'trStyle', 'pStyle');
									$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
									$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
									$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
									$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
		...
		$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
		$section->addTextBreak(1);
		if($report_compitative_landscape){
		...
		foreach($rd_companies as $companies)
		{
			...
			$section->addImage('images/new/sample-table.png', array('width'=>760, 'height'=>220, 'align'=>'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			break;
		}
		$section->addTextBreak(1);
		...
		$section->addImage('images/new/our-expertise.jpg',['positioning' => 'relative','marginTop' => 0,'marginLeft' => 1,'width' => 850,'height' => 605,'wrappingStyle' => 'tight']);
		$section->addImage('images/new/infinium_disclaimer.jpg',['positioning' => 'relative','marginTop' => 0,'marginLeft' => 1,'width' => 850,'height' => 605,'wrappingStyle' => 'tight']);
	/* /. Sample Pages Chapter 6 - Company Profile */
			...
			$new_file_name4=str_replace("--","-", $new_file_name3);
			$objWriter = PHPWord_IOFactory::createWriter($PHPWord, 'Word2007');
			$filename = "Sample-".htmlspecialchars($new_file_name4)."-Report.docx";
			$objWriter->save($filename);
			header('Content-Description: File Transfer');
			header('Content-Type: application/octet-stream');
			header('Content-Disposition: attachment; filename='.$filename);
		...
		if($this->session->userdata('logged_in')){
			...
			foreach($main_segments as $segments)
			{
				...
				foreach($sub_segments as $subsegments)
				{
					$child_segments = $this->RdData_model->get_rd_segments($report_id, $subsegments->id);
					if($child_segments){
						$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($segments->name).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
						$num++;

						if($Volume_unit)
						{
							$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($segments->name).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
							$num++;
						}
						/* --- child segment--- */
						foreach($child_segments as $childsegments)
						{
							$sub_child_segments = $this->RdData_model->get_rd_segments($report_id, $childsegments->id);
							if($sub_child_segments){
								$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$subsegments->name)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
								$num++;
								if($Volume_unit)
								{
									$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." FOR ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$subsegments->name)).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
									$num++;
								}
								/* --- sub child segment--- */							
								if($sub_child_segments){
									foreach($sub_child_segments as $subchildsegments)
									{
										$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title.' BY '.$subchildsegments->name))." BY ".htmlspecialchars(strtoupper($scope_type)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
										$num++;
										if($Volume_unit)
										{
											$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title.' BY '.$subchildsegments->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
											$num++;
										}
									}
								}
								...
								$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$scope_type)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
								$num++;
								if($Volume_unit)
								{
									$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." FOR ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$scope_type)).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
									$num++;
								}
							}							
						}
						...
						if($Volume_unit)
						{
							$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title))." BY ".htmlspecialchars(strtoupper($subsegments->name.' BY '.$scope_type)).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
							$num++;
						}
					}
				}
			...
			foreach($main_segments as $segments)
			{
				...
				foreach($sub_segments as $subsegments)
				{
					$child_segments = $this->RdData_model->get_rd_segments($report_id, $subsegments->id);
					if($child_segments){
						$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($segments->name).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
						$number++;
						if($Volume_unit)
						{
							$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($segments->name).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
							$number++;
						}	
						/* --- child segment---- */	
						foreach($child_segments as $childsegments)
						{
							$sub_child_segments = $this->RdData_model->get_rd_segments($report_id, $childsegments->id);	
							if($sub_child_segments){
							$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$subsegments->name)).", ".$Base_year." - ".$forecast_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
							$number++;
							if($Volume_unit)
							{
								$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$subsegments->name)).", ".$Base_year." - ".$forecast_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
								$number++;
							}							
								/* --- sub child segment---- */
								foreach($sub_child_segments as $subchildsegments)
								{
									$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($subchildsegments->name.' BY '.$scope_type)).", ".$Base_year." - ".$forecast_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
									$number++;
									if($Volume_unit)
									{
										$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." FOR ".htmlspecialchars(strtoupper($$subchildsegments->name.' BY '.$scope_type)).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
										$number++;
									}
								} /* ./ --- sub child segment---- */
							} else{
								$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$scope_type)).", ".$Base_year." - ".$forecast_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
								$number++;
								if($Volume_unit)
								{
									$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$report_title))." BY ".htmlspecialchars(strtoupper($childsegments->name.' BY '.$scope_type)).", ".$Base_year." - ".$forecast_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
									$number++;
								}
							}
						}	/* ./ --- child segment---- */				
					}else{
						$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(strtoupper($Value_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
						$number++;
						if($Volume_unit)
						{
							$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
							$number++;
						}
					}		
				}
			...
			if($scope_name == 'Global'){
				$section->addImage('images/primaries_breakdown.png', array('width'=>770, 'height'=>280, 'align'=>'center'));
			} else {
				$section->addImage('images/primaries_breakdown_country.png', array('width'=>770, 'height'=>250, 'align'=>'center'));
			}			
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addText(htmlspecialchars('Primary interviews not only help in data validation but also provide critical insights into the market, current business scenario, and future expectations and enhance the quality of our reports. In addition to analyzing the current and historical trends, our analysts predict where the market is headed, over the next five years.'), 'r2Style', 'paragraphStyle');
			$section->addTextBreak(1);
			$section->addText(htmlspecialchars('Primary Sources Considered During This Particular Study:'), array('align'=>'left', 'bold'=>true, 'name'=>'Franklin Gothic Medium', 'color'=>'#000', 'size' => 10.5));
			$section->addImage('images/primary_sources.png', array('width'=>640, 'height'=>380, 'align'=>'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			$section->addListItem(htmlspecialchars('	Secondary Research:'), 3, 'childSubpoint', $listStyle, 'Head 3');
			$secondary_research_para2 = explode('\n', $secondary_research_para1);
			...
			if($scope_name == 'Global'){							
				...
				$section->addImage('images/research_approaches_bottom_up_region.png', array('width'=>520, 'height'=>250, 'align'=>'center'));
			}
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			$section->addText(htmlspecialchars('RESEARCH APPROACHES - TOP-DOWN'), 'figureNameStyle', 'Figure _ Title');			 
			if($scope_name == 'Global'){
				...
				$section->addImage('images/research_approaches_top_down_2023_2030_region.png', array('width'=>520, 'height'=>250, 'align'=>'center'));
			}
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			$section->addText(htmlspecialchars($Get_research_approaches_para1_6), 'r2Style', 'paragraphStyle');
			...
			/* Sample Pages Chapter 2 */
			$section->addListItem('Executive Summary', 0, 'chaptHeading', $listStyle, 'Main Heading');
			$section->addListItem(htmlspecialchars(ucwords($Report_title_scope, " \t\r\n\f\v'"))." Highlights, (USD ".htmlspecialchars(ucwords($Value_unit)).")",1, 'subPoint', $listStyle, 'Head 1');
			$section->addText(htmlspecialchars($Get_executive_summary_para1_5), 'r2Style', 'paragraphStyle');
			$section->addTextBreak(1);
			$table2 = htmlspecialchars(strtoupper($Report_title_scope))." HIGHLIGHTS";
			$section->addText(htmlspecialchars($table2), 'tableNameStyle', 'Table_Title');
			...
			if($stack != NULL){
    			...
    			for($i = 1; $i <= count($stack); $i++) {
    				...
    				{						
    					...
    					{	
    						...
    						foreach($main_segments3 as $segments3)
    						{
    							...
    							$mainseg3 = $segments3->name;
    							$table->addRow();
    							$table->addCell(3500, $styleCellColor)->addText(htmlspecialchars(ucwords($report_title, " \t\r\n\f\v'"))." by ".htmlspecialchars($mainseg3)." Value (USD ".ucwords($Value_unit).")",'trStyle1', 'Table1_Left');
    							$table_cell = $table->addCell(3500, $styleCellColor);
    							$text="";
    							$x=1;
    							$sub_segments3 = $this->RdData_model->get_rd_segments($report_id, $segments3->id);	
    				...
    				else if( $i == count($stack ) ) // Last Element
    				{
    					...
    					$table->addRow();
    					$table_cell = $table->addCell(3500, $styleLastCell);
						$table_cell->addText(htmlspecialchars(ucwords($Report_title_scope))." by ".$scope_type." (USD ".htmlspecialchars(ucwords($Value_unit)).")", 'trStyle1', 'pStyle');
    					$table_cell = $table->addCell(3500, $styleLastCell);
						$get_scope_countries = $this->RdData_model->get_scope_regions($scope_id); 
						// var_dump($get_scope_countries); die;
    					foreach($get_scope_countries as $scope_countries)
		...
		if($scope_name == 'Global'){
			...
			/* Bar chart figure */
				if($Volume_unit) {
					...
					$section->addText(htmlspecialchars(strtoupper($report_name_scope))." BY ".htmlspecialchars(strtoupper($segments5->name)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'figureNameStyle', 'Figure _ Title');
				}
				/* Figure */
				if($forecast_period=='2021-2027') {
					$section->addImage('images/sample/xyz-market-by-segment-revenue-share-2019-2027-bar.png', array('width'=>760, 'height'=>240, 'align'=>'center'));
			...
			/* Table Writeup -  Main Segment */
				/* value based table */
				$section->addText(htmlspecialchars(strtoupper($report_name_scope))." BY ".htmlspecialchars(strtoupper($segments5->name)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'tableNameStyle', 'Table_Title');
				// Define table style arrays 				
				// Add table
				$table = $section->addTable('myOwnTableStyle');
				// Add row
				...
				foreach($sub_segments5_2 as $subsegments5_2)
				{
					...
					if($child_segments3){
					/* adding figure - sub segment */					
					$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subsegments5_2->name))." BY ".strtoupper($segments5->name).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'figureNameStyle', 'Figure _ Title');
					...
					/* Table Writeup - Sub Segment */
					/* value based table */
					$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subsegments5_2->name))." BY ".strtoupper($segments5->name).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'tableNameStyle', 'Table_Title');
					// Add table
					$table = $section->addTable('myOwnTableStyle');
					// Add row
					$table->addRow(70);
				...
				/* ./ Table Writeup - Sub Segment */
					...
					/* adding figure */					
					$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subsegments5_2->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'figureNameStyle', 'Figure _ Title');
					$section->addImage('images/sample-figure.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
					$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
					$section->addTextBreak(1);
					if($Volume_unit)
					...
					/* Table Writeup - Sub Segment */
					/* value based table */
					$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subsegments5_2->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'tableNameStyle', 'Table_Title');
					// Add table
					$table = $section->addTable('myOwnTableStyle');
					// Add row
					$table->addRow(70);
				...
				{					    
					foreach($child_segments3 as $childsegments3)
					{
						...
						/* Child Segment - Table & Figure Writing started*/								
							...
							if($sub_child_segments3){	
								/* child segment figure title */
								$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($child_segment3 ))." BY ".htmlspecialchars(strtoupper($subsegments5_2->name)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'figureNameStyle', 'Figure _ Title');
								$section->addImage('images/sample-figure.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
								$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
								$section->addTextBreak(1);
								if($Volume_unit)
								...
								/* value based table */															
								$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($child_segment3))." BY ".htmlspecialchars(strtoupper($subsegments5_2->name)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'tableNameStyle', 'Table_Title');
								// Add table
								$table = $section->addTable('myOwnTableStyle');
								// Add row
								$table->addRow(70);
								...
								foreach($sub_child_segments3 as $subchildsegments3)
								{
									...
									$section->addTextBreak(1);		
									/* sample-figure */
									$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subchild_segment3))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'figureNameStyle', 'Figure _ Title');
									$section->addImage('images/sample-figure.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
									$section->addTextBreak(1);
									/* value basedtTable */
									$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($subchild_segment3))." BY ".htmlspecialchars(strtoupper($scope_type)).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'tableNameStyle', 'Table_Title');
									// Define table style arrays
									$table = $section->addTable('myOwnTableStyle');
									// Add row
									$table->addRow(70);
									...
									foreach($get_scope_regions31 as $scope_regions31)
									{
										...
										$styleCellColor = array('bgColor'=>$bg_color);
										$table->addRow();
										$table->addCell(2500, $styleCellColor)->addText(htmlspecialchars(ucwords($scope_regions31->name)), 'trStyle', 'pStyle');
										$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
										$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
										$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
										$table->addCell(1600, $styleCellColor)->addText("XX.X", 'trStyle', 'Table 1_ Right Center');
								...
								/* Child segment table & figure writing */
								$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($child_segment3))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'figureNameStyle', 'Figure _ Title');
								$section->addImage('images/sample-figure.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
								$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
								$section->addTextBreak(1);
								if($Volume_unit)
								...
								$section->addText(htmlspecialchars(strtoupper($scope_name.' '.$report_name))." BY ".htmlspecialchars(strtoupper($child_segment3))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'tableNameStyle', 'Table_Title');
								// Add table
								$table = $section->addTable('myOwnTableStyle');
								// Add row
								$table->addRow(70);
			...
			/* value based table */
			$section->addText(htmlspecialchars(strtoupper($report_name_scope))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'tableNameStyle', 'Table_Title');
			// Define table style arrays
			$table = $section->addTable('myOwnTableStyle');
			// Add row
			$table->addRow(70);
			...
			if($scoperegion4 != NULL){
				...
				{					
					...
					foreach($main_segments6 as $segments6)
					{
						...
						$section->addListItem(htmlspecialchars($region_report_title1), 2, 'childPoint', $listStyle, 'Head 2');						
						if($Volume_unit){
							$section->addText(htmlspecialchars(strtoupper($region_report_title1))." ".$Base_year." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'figureNameStyle', 'Figure _ Title');
							...
							$section->addText(htmlspecialchars(strtoupper($region_report_title1))." ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'figureNameStyle', 'Figure _ Title');
				...
				if($scope_name == 'Global'){
					if($scoperegion4[$i] == "RoW")
					{
						$region_name1 = ltrim(rtrim($scoperegion4[$i])).' '.ltrim(rtrim(ucwords($report_name))).', by Sub-region';
						$section->addListItem(htmlspecialchars($region_name1), 2, 'childPoint', $listStyle, 'Head 2');						
						$section->addText(htmlspecialchars(strtoupper($region_name1))." ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'figureNameStyle', 'Figure _ Title');
							...
							for($r = 0; $r < $x ; $r++)
							{
								...
								foreach($main_segments7 as $segments7)
								{
									...
									if($Volume_unit)
									{
										...
										$section->addText(htmlspecialchars(strtoupper($country_title))." ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars(ucwords($Value_unit)).')', 'figureNameStyle', 'Figure _ Title');
										if($forecast_period =='2021-2027')
										{
											$section->addImage('images/sample/xyz-market-by-segment-revenue-share-2019-2027-bar.png', array('width'=>760, 'height'=>240, 'align'=>'center'));
										} else if($forecast_period =='2022-2028') {
	...
	public function mail_draft(){
		if($this->session->userdata('logged_in')){
			...
			$new_file_name3=str_replace("&amp;","and", $new_file_name2);
			$new_file_name4=str_replace("--","-", $new_file_name3);
			$filename = "Mail-Draft-".htmlspecialchars($new_file_name4).".docx";
			header('Content-Disposition: attachment; filename='.$filename);
			ob_clean();
