...
class Generate_rd extends CI_Controller {    
	...
	{		
		...
		if($this->session->userdata('logged_in')){
			...
			$pr2_description = $this->RdData_model->get_pr2_data($report_id);
			//  var_dump($pr2_description->description); die;
			/*  foreach($pr2_description as $pr2description)
			{
				$new_description = ucwords($pr2description->description, " \t\r\n\f\v'");	
			//  $section->addListItem(htmlspecialchars($new_description), 0);
				$section->addText(htmlspecialchars($new_description), array('align'=>'left','bold'=>true,'name'=>'Verdana'));
			} */
			 $section->addText(htmlspecialchars($pr2_description->description), array('align'=>'left','bold'=>false,'name'=>'Verdana'));
			//  var_dump($new_description); die;
			$section->addTextBreak(1);
			...
			$new_file_name4=str_replace("--","-", $new_file_name3);
			$objWriter = PHPWord_IOFactory::createWriter($PHPWord, 'Word2007');
			$filename = "".htmlspecialchars($new_file_name4)."-PR2.docx";
			$objWriter->save($filename);
			header('Content-Description: File Transfer');
			header('Content-Type: application/octet-stream');
			header('Content-Disposition: attachment; filename='.$filename);
	...
	public function merge_sample_pages(){
		if($this->session->userdata('logged_in')){
			...
			$templateFile  = "resources/sample_template.docx";
			$generatedFile = "resources/Global-Market-PR2.docx";
	...
	public function sample_pages(){
		...
		if($this->session->userdata('logged_in')){
			...
			$PHPWord = new PhpWord();
			// $section = $PHPWord->addSection();
			$section = $PHPWord->addSection(array('marginLeft'=>120, 'marginRight'=>120,'marginTop'=>1140,'marginBottom'=>950, 'orientation' => 'landscape'));
			 
			/*----------------------Header -------------------------------*/	
			$PHPWord->addParagraphStyle('headerTitle', array('align'=>'left', 'spacing' => 72, 'spaceBefore' => 140, 'spaceAfter' => 140));
			...
			$table->addRow();
			$table->addCell(17000, array('borderBottomSize' => 15, 'borderBottomColor' => '#E59C24', 'marginLeft'=>120, 'marginRight'=>120,'marginTop'=>1140,'marginBottom'=>950))->addText(htmlspecialchars('first'));
			$section->addText(htmlspecialchars(strtoupper($report_title)), array('align'=>'left','name'=>'Roboto Condensed', 'color'=>'#78777C', 'size' => 28, 'marginLeft'=>720, 'marginRight'=>720,'marginTop'=>1440,'marginBottom'=>1150));
			...
			/* $new_file_name4="sample-pages";
			$objWriter = PHPWord_IOFactory::createWriter($PHPWord, 'Word2007');
			$filename = "".htmlspecialchars($new_file_name4).".docx";
			$objWriter->save($filename);
			header('Content-Description: File Transfer');
			header('Content-Type: application/octet-stream');
			header('Content-Disposition: attachment; filename='.$filename);
