    ...
    public function sample_pages($report_id){        
        if($this->session->userdata('logged_in')){
			...
			{				
				...
				$section->addText('			'.$cmpt.'.'.$cmpsub.'.'.$cmpsubchild.'.3	Financial Snapshot', 'childpoint', 'P-Style');
				$section->addText('			'.$cmpt.'.'.$cmpsub.'.'.$cmpsubchild.'.4	Product Portfolio', 'childpoint', 'P-Style');	
				$section->addText('			'.$cmpt.'.'.$cmpsub.'.'.$cmpsubchild.'.5	Recent Developments', 'childpoint', 'P-Style');	
				$cmpsubchild++;
			}
			$section->addPageBreak();
			/* ./ TOC Chapter 5 Company Profiles */
