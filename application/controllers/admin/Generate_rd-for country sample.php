						...
						foreach($child_segments as $childsegments)
						{
							...
							if($sub_child_segments){
								foreach($sub_child_segments as $subchildsegments)
								{
									...
									if($Volume_unit)
									{
										$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name." ".$subchildsegments->name))." MARKET FOR ".htmlspecialchars(strtoupper($childsegments->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
										$num++;
									}
								}
							}
						...
						foreach($child_segments as $childsegments)
						{
							...
							if($sub_child_segments){
								foreach($sub_child_segments as $subchildsegments)
								{
									...
									if($Volume_unit)
									{
										$section->addText('FIGURE '.$number.'   '.htmlspecialchars(strtoupper($scope_name." ".$subchildsegments->name))." MARKET FOR ".htmlspecialchars(strtoupper($childsegments->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
										$number++;
									}
								}
							} /* ./ --- sub child segment---- */
						...
						foreach($child_segments3 as $childsegments3)
						{
							...
							{					    
								foreach($sub_child_segments3 as $subchildsegments3)
								{
									...
									/* volume based table */
									if($Volume_unit){								
										$section->addText(htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($child_segment3->name))." FOR ".htmlspecialchars(strtoupper($subchild_segment3))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' (USD '.htmlspecialchars($Volume_unit).')', 'tableNameStyle', 'Table_Title');
										// Define table style arrays
										$table = $section->addTable('myOwnTableStyle');
										// Add row
										$table->addRow(70);
