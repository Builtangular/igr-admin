		...
		if($this->session->userdata('logged_in')){
			...
			foreach($main_segments as $segments)
			{
				...
				foreach($sub_segments as $subsegments)
				{
					...
					if($child_segments){
						...
						if($Volume_unit)
						{
							$section->addText('TABLE '.$num.'   '.htmlspecialchars(strtoupper($scope_name))." ".htmlspecialchars(strtoupper($report_title.' BY '.$subsegments->name))." BY ".strtoupper($scope_type).", ".$analysis_from." - ".$analysis_to.' ('.htmlspecialchars(strtoupper($Volume_unit)).')', array('align'=>'left','name'=>'Franklin Gothic Medium','size'=>10,'color'=>'#78777C'));
							$num++;
						}
						/* --- child segment--- */
						foreach($child_segments as $childsegments)
			...
			for($i = 1; $i <= 9; $i++) {
				...
				else if($i==6)
				{
					...
					$table->addCell(6500)->addText("Compounded Annual Growth Gate", 'trStyle', 'pStyle');
				}
				else if($i==7)
				{
					$table->addRow();
