...
class CountrySample extends CI_Controller {    
    ...
    public function sample_pages($report_id){        
        if($this->session->userdata('logged_in')){
			...
			/* Cover page */
            $title_len = strlen($report_title_scope); 
			if($title_len < 33){
				$font_size = 30;
			}else{
				$font_size = 22;
			...
			$section->addText(htmlspecialchars(strtoupper('								')), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#00435E', 'size' => 30, 'bold' => true));
			$section->addText(htmlspecialchars(strtoupper('								')), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#00435E', 'size' => 30, 'bold' => true));			
			$section->addText(htmlspecialchars(strtoupper('								'.$report_title_scope)), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#00435E', 'size' => $font_size, 'bold' => true));
			$section->addText('								PROSPECTS, TRENDS ANALYSIS, MARKET SIZE AND FORECASTS UP TO '.strtoupper($forecast_to), array('align'=>'justified','name'=>'Roboto Condensed', 'color'=>'#7F7F7F', 'size' => 11));
			...
			$PHPWord->addFontStyle('myOwnStyle', array('bold'=>false,'spaceAfter'=>0, 'name'=>'Franklin Gothic Medium','size' => 15, 'color'=>'#E59C24', 'line-height'=> 1.5));
			...
			$PHPWord->addParagraphStyle('P-Style', array('spacing' => 0, 'spaceAfter'=>100, 'name'=>'Franklin Gothic Medium', 'size' => 12.5,'color'=>'#78777C', 'indentation' => array('left' => 0.9, 'right' => 0.27, 'hanging' => 0.65)));
			$PHPWord->addParagraphStyle('PStyle', array('spacing' => 120, 'spaceBefore' => 120, 'spaceAfter'=>0, 'name'=>'Franklin Gothic Medium', 'size' => 12.5,'color'=>'#78777C', 'indentation' => array('left' => 0.9, 'right' => 0.27, 'hanging' => 0.65)));
			...
			{
				$listStyleBullet = array('listType'=>PhpOffice\PhpWord\Style\ListItem::TYPE_BULLET_FILLED, 'name'=>'Franklin Gothic Medium', 'size' => 12.5, 'color'=>'#78777C', 'spaceAfter'=>0, 'spaceBefore'=> 0,  'spacing'=>0);
				$listStyleBulletEmpty = array('listType'=>PhpOffice\PhpWord\Style\ListItem::TYPE_BULLET_EMPTY , 'name'=>'Franklin Gothic Medium', 'size' => 12.5, 'color'=>'#78777C', 'spaceAfter'=>0, 'spaceBefore'=> 0,  'spacing'=>0);
			...
			$styleCellColor = array('bgColor'=>'F2F2F2');
			$styleLastCell = array('bgColor'=>'F2F2F2', 'borderBottomColor'=>'#78777C', 'borderBottomSize'=>2);
			...
			for($i = 1; $i <= 9; $i++) {
				...
				{
					$table->addRow();
					$table->addCell(3000, $styleRightCellColor)->addText(htmlspecialchars(ucwords($report_title_scope, " \t\r\n\f\v'"))." (Definition)", 'trStyle1', 'pStyle');
					$table->addCell(6500)->addText(htmlspecialchars($report_definition), 'trStyle', 'pStyle');
				}
				else if($i==2)
				{					
					$table->addRow();
					$table->addCell(3000, $styleRightCellColor)->addText("Defining Market Segments", 'trStyle1', 'Table1_Left');
					$table->addCell(6500, $styleCellColor)->addText($market_segments, 'trStyle', 'pStyle');
				}
			}
			...
			foreach($main_segments2 as $segments2)
			{
				...
				$table->addRow();
				$table->addCell(3000, $styleRightCellColor)->addText(htmlspecialchars($segments2->name), 'trStyle1', 'Table1_Left');
			...
			$table->addRow();
			$table->addCell(3000, $styleRightCellColor)->addText("Revenue", 'trStyle1', 'Table1_Left');
			...
			$section->addText(htmlspecialchars('Breakdown of the Profiles of Primaries as Follows:'), array('align'=>'left', 'bold'=>true, 'name'=>'Franklin Gothic Medium', 'color'=>'#000', 'size' => 12.5));				
			$section->addImage('images/new/primaries_breakdown_for_country_sample.png', array('width'=>410, 'height'=>230, 'align'=>'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addText(htmlspecialchars('Primary interviews not only help in data validation but also provide critical insights into the market, current business scenario, and future expectations and enhance the quality of our reports. In addition to analyzing the current and historical trends, our analysts predict where the market is headed, over the next five years.'), 'r2Style', 'paragraphStyle');
			$section->addTextBreak(1);
			$section->addText(htmlspecialchars('Primary Sources Considered During this Particular Study:'), array('align'=>'left', 'bold'=>true, 'name'=>'Franklin Gothic Medium', 'color'=>'#000', 'size' => 12.5));
			$section->addImage('images/new/primary_sources.png', array('width'=>460, 'height'=>280, 'align'=>'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			...
			$section->addImage('images/new/research_approaches_bottom_up_country.png', array('width'=>780, 'height'=>320, 'align'=>'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
    			...
    			for($i = 1; $i <= count($stack); $i++) {
    				...
    				else if($i==2) 
    				{
    					...
    					if($Volume_unit){
                            ...
                            $table_cell = $table->addCell(7000, $styleMergeCell);
                            $table_cell->addText("Value: ".$value_cagr, 'trStyle1', 'pStyle');
    						$table_cell->addText("Volume:".$volume_based_cagr, 'trStyle1', 'pStyle'); 
    					}else{
			...
			$section->addImage('images/pest_analysis_country.png', array('width'=>760, 'height'=>350, 'align'=>'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			...
			$section->addImage('images/porters_diamond_model_country.png', array('width'=>760, 'height'=>350, 'align'=>'center'));
			$section->addText('Source: Infinium Global Research Analysis', 'Source Note', 'sourceStyle');
			$section->addTextBreak(1);
			/* style */
			...
			/* Bar chart figure */
				...
				/* Figure */
				if($forecast_period=='2023-2030')	{
					$section->addImage('images/new/xyz-market-by-segment-revenue-share-2021-2030-bar.png', array('width'=>760, 'height'=>260, 'align'=>'center'));
				}else {
