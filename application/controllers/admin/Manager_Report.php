...
class Manager_Report extends CI_Controller {    
	...
	public function manager_published_rd(){
    ...
    public function drafts(){
		if($this->session->userdata('logged_in')){
			...
			$data['Role_id'] = $session_data['Role_id'];
			$status = 1;
			$data['Global_Rds']= $this->Data_model->get_drafted_manager_global_rd($status);
			$this->load->view('admin/draft/list',$data);			
		}else{			
			$this->load->view('admin/login');
		}
	}
	public function manager_processed_rd(){
		if($this->session->userdata('logged_in')){
			$session_data = $this->session->userdata('logged_in');
			$data['Login_user_name']=$session_data['Login_user_name'];	
			$data['Role_id']=$session_data['Role_id'];
			$status = 2;
			$data['Global_Rds']= $this->Data_model->get_manager_global_rds($status);
